...
DEF_SEM(BOUND, R8W cond, S1 src1, S2 src2) {
  ...
  HYPER_CALL = AsynchHyperCall::kX86Bound;
  INTERRUPT_VECTOR = 5;
...
DEF_ISEL_SEM(INT_IMMb, I8 num) {
  ...
  HYPER_CALL = AsynchHyperCall::kX86IntN;
}

DEF_ISEL_SEM(INT1) {
  INTERRUPT_VECTOR = 1;
  HYPER_CALL = AsynchHyperCall::kX86Int1;
}

DEF_ISEL_SEM(INT3) {
  INTERRUPT_VECTOR = 3;
  HYPER_CALL = AsynchHyperCall::kX86Int3;
}
...
DEF_ISEL_SEM(INTO, R8W cond) {
  Write(cond, FLAG_OF);
  INTERRUPT_VECTOR = 4;
  HYPER_CALL = AsynchHyperCall::kX86IntO;
}
