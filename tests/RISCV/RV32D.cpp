...
TEST(RISCV32, FaddD_Basic) {
  ...
  riscv::test::ExecuteOne<remill::ArchName::kArchRISCV32>(
      lifter, "riscv32_fadd_d_f3_f1_f2", riscv::Bytes32(word), /*addr=*/0x1000,
      &st, &mem);
...
TEST(RISCV32, FldAndFsd_RoundTrip) {
  ...
  RISCVState st = {};
  st.pc.dword = 0x2000u;
  st.gpr.x10.dword = static_cast<uint32_t>(load_addr);
  st.gpr.x11.dword = static_cast<uint32_t>(store_addr);
  ...
  const auto fld =
      riscv::EncodeI(riscv::kOpcodeLoadFp, /*rd=*/1, /*funct3=*/0x3,
                     /*rs1=*/10, /*imm12=*/0);
  riscv::test::ExecuteOne<remill::ArchName::kArchRISCV32>(
      lifter, "riscv32_fld_f1_x10_0", riscv::Bytes32(fld), /*addr=*/0x2000, &st,
      &mem);
  ...
  const auto fsd =
      riscv::EncodeS(riscv::kOpcodeStoreFp, /*funct3=*/0x3, /*rs1=*/11,
                     /*rs2=*/1, /*imm12=*/0);
  riscv::test::ExecuteOne<remill::ArchName::kArchRISCV32>(
      lifter, "riscv32_fsd_f1_x11_0", riscv::Bytes32(fsd), /*addr=*/0x2004, &st,
      &mem);
