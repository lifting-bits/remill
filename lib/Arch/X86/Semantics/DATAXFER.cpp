...
// MOVMSKPD: extract sign bits (bit 63) of 2 double-precision floats into GPR32
template <typename D, typename S>
DEF_SEM(MOVMSKPD, D dst, S src) {
  auto src_vec = UReadV64(src);
  auto s0 = UExtractV64(src_vec, 0);
  auto s1 = UExtractV64(src_vec, 1);
  auto r32 = UOr(static_cast<uint32_t>(UShr(s0, 63_u64)),
                 UShl(static_cast<uint32_t>(UShr(s1, 63_u64)), 1_u32));
  WriteZExt(dst, r32);
  return memory;
}
...
DEF_ISEL(MOVD_MMXq_MEMd) = MOVD<V64W, MV32>;  // Zero extends.
DEF_ISEL(MOVD_MMXq_GPR32) = MOVD<V64W, V32>;  // Zero extends.
...
DEF_ISEL(MOVZX_GPRv_MEMw_16) = MOVZX<R16W, M16>;
DEF_ISEL(MOVZX_GPRv_MEMw_32) = MOVZX<R32W, M16>;
IF_64BIT(DEF_ISEL(MOVZX_GPRv_MEMw_64) = MOVZX<R64W, M16>;)
DEF_ISEL(MOVZX_GPRv_GPR16) = MOVZX<R16W, R16>;
DEF_ISEL(MOVZX_GPRv_GPR16_16) = MOVZX<R16W, R16>;
...
DEF_ISEL(MOVSX_GPRv_MEMw_16) = MOVSX<R16W, M16, int16_t>;
...
DEF_ISEL(MOVSX_GPRv_GPR16) = MOVSX<R16W, R16, int16_t>;
DEF_ISEL(MOVSX_GPRv_GPR16_16) = MOVSX<R16W, R16, int16_t>;
DEF_ISEL(MOVSX_GPRv_GPR16_32) = MOVSX<R32W, R16, int32_t>;
IF_64BIT(DEF_ISEL(MOVSX_GPRv_GPR16_64) = MOVSX<R64W, R16, int64_t>;)

DEF_ISEL(MOVSXD_GPRv_GPRz_16) = MOVSX<R16W, R16, int16_t>;
DEF_ISEL(MOVSXD_GPRv_GPRz_32) = MOVSX<R32W, R32, int32_t>;

IF_64BIT(DEF_ISEL(MOVSXD_GPR64_MEMd) = MOVSX<R64W, M32, int64_t>;)
IF_64BIT(DEF_ISEL(MOVSXD_GPRv_MEMd_32) = MOVSX<R32W, M32, int32_t>;)
IF_64BIT(DEF_ISEL(MOVSXD_GPRv_GPR32_32) = MOVSX<R32W, R32, int32_t>;)
