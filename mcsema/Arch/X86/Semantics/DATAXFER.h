...
DEF_ISEL(MOVLPD_XMMsd_MEMq) = LEGACY_COPY_QWORD<V128W, MV64>;
DEF_ISEL(MOVLPD_MEMq_XMMsd) = COPY_QWORD<M64W, V128>;
#if HAS_FEATURE_AVX
DEF_ISEL(VMOVLPD_MEMq_XMMq) = COPY_QWORD<M64W, V128>;
DEF_ISEL_SEM(VMOVLPD_XMMdq_XMMdq_MEMq, VV128W dst, VV128 src1, M64 src2) {
  auto src = R(src1);
  src.qwords[0] = R(src2);
  W(dst) = src;
}
