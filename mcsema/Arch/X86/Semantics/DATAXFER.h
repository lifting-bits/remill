...
DEF_SEM(COPY_QWORD, D1 dst, S1 src_vec) {
  W(dst) = R(src_vec).qwords[0];  // Zero-extends.
}

template <typename D1, typename S1>
DEF_SEM(LEGACY_COPY_QWORD, D1 dst, S1 src_vec) {
  W(DownCast<V64W>(dst)) = R(src_vec).qwords[0];  // No zero-extension.
}

template <typename D1, typename S1>
DEF_SEM(COPY_DWORD, D1 dst, S1 src_vec) {
  W(dst) = R(src_vec).dwords[0];
}

template <typename D1, typename S1>
DEF_SEM(LEGACY_COPY_DWORD, D1 dst, S1 src_vec) {
  W(DownCast<V32W>(dst)) = R(src_vec).dwords[0];  // No zero-extension.
}
...
DEF_ISEL(MOVQ_XMMdq_MEMq_0F6E) = COPY_QWORD<V128W, MV64>;
IF_64BIT(DEF_ISEL(MOVQ_XMMdq_GPR64) = MOV<V128W, R64>;)
DEF_ISEL(MOVQ_MEMq_XMMq_0F7E) = COPY_QWORD<M64W, V128>;
IF_64BIT(DEF_ISEL(MOVQ_GPR64_XMMq) = COPY_QWORD<R64W, V128>;)
DEF_ISEL(MOVQ_MEMq_XMMq_0FD6) = COPY_QWORD<M64W, V128>;
DEF_ISEL(MOVQ_XMMdq_XMMq_0FD6) = COPY_QWORD<V128W, V128>;
DEF_ISEL(MOVQ_XMMdq_MEMq_0F7E) = COPY_QWORD<V128W, MV64>;
DEF_ISEL(MOVQ_XMMdq_XMMq_0F7E) = COPY_QWORD<V128W, V128>;

#if HAS_FEATURE_AVX
DEF_ISEL(VMOVQ_XMMdq_MEMq_6E) = COPY_QWORD<VV128W, MV64>;
IF_64BIT(DEF_ISEL(VMOVQ_XMMdq_GPR64q) = MOV<VV128W, R64>;)
DEF_ISEL(VMOVQ_MEMq_XMMq_7E) = COPY_QWORD<M64W, V128>;
IF_64BIT(DEF_ISEL(VMOVQ_GPR64q_XMMq) = COPY_QWORD<R64W, V128>;)
DEF_ISEL(VMOVQ_XMMdq_MEMq_7E) = COPY_QWORD<VV128W, MV64>;
DEF_ISEL(VMOVQ_XMMdq_XMMq_7E) = COPY_QWORD<VV128W, V128>;
DEF_ISEL(VMOVQ_MEMq_XMMq_D6) = COPY_QWORD<M64W, V128>;
DEF_ISEL(VMOVQ_XMMdq_XMMq_D6) = COPY_QWORD<VV128W, V128>;

#if HAS_FEATURE_AVX512
DEF_ISEL(VMOVQ_XMMu64_MEMu64_AVX512) = COPY_QWORD<VV128W, MV64>;
IF_64BIT(DEF_ISEL(VMOVQ_GPR64u64_XMMu64_AVX512) = COPY_QWORD<R64W, V128>;)
IF_64BIT(DEF_ISEL(VMOVQ_XMMu64_GPR64u64_AVX512) = MOV<VV128W, R64>;)
DEF_ISEL(VMOVQ_XMMu64_XMMu64_AVX512) = COPY_QWORD<VV128W, V128>;
DEF_ISEL(VMOVQ_MEMu64_XMMu64_AVX512) = COPY_QWORD<M64W, V128>;
#endif  // HAS_FEATURE_AVX512
#endif  // HAS_FEATURE_AVX

DEF_ISEL(MOVD_XMMdq_MEMd) = COPY_DWORD<V128W, MV32>;
DEF_ISEL(MOVD_XMMdq_GPR32) = MOV<V128W, R32>;  // Zero extends.
DEF_ISEL(MOVD_MEMd_XMMd) = COPY_DWORD<M32W, V128>;
DEF_ISEL(MOVD_GPR32_XMMd) = COPY_DWORD<R32W, V128>;

#if HAS_FEATURE_AVX
DEF_ISEL(VMOVD_XMMdq_MEMd) = MOV<VV128W, M32>;
DEF_ISEL(VMOVD_XMMdq_GPR32d) = MOV<VV128W, R32>;
DEF_ISEL(VMOVD_MEMd_XMMd) = COPY_DWORD<M32W, V128>;
DEF_ISEL(VMOVD_GPR32d_XMMd) = COPY_DWORD<R32W, V128>;
#if HAS_FEATURE_AVX512
DEF_ISEL(VMOVD_XMMu32_MEMu32_AVX512) = MOV<VV128W, M32>;
DEF_ISEL(VMOVD_XMMu32_GPR32u32_AVX512) = MOV<VV128W, R32>;
DEF_ISEL(VMOVD_MEMu32_XMMu32_AVX512) = COPY_DWORD<M32W, V128>;
DEF_ISEL(VMOVD_GPR32u32_XMMu32_AVX512) = COPY_DWORD<R32W, V128>;
#endif  // HAS_FEATURE_AVX512
#endif  // HAS_FEATURE_AVX

DEF_ISEL(MOVLPS_MEMq_XMMps) = COPY_QWORD<M64W, V128>;
DEF_ISEL_SEM(MOVLPS_XMMq_MEMq, V128W dst, M64 src) {
  W(DownCast<V64W>(dst)) = R(src);  // Legacy, no zext.
}

#if HAS_FEATURE_AVX
namespace {
template <typename S2>
DEF_SEM(VMOVQ_XMM_XMM_SRC2, VV128W dst, V128 src1, S2 src2) {
  auto src = R(src1);
  src.qwords[0] = R(src2).qwords[0];
  W(dst) = src;
}

template <typename S2>
DEF_SEM(VMOVD_XMM_XMM_SRC2, VV128W dst, V128 src1, S2 src2) {
  auto src = R(src1);
  src.dwords[0] = R(src2).dwords[0];
  W(dst) = src;
}
}  // namespace

DEF_ISEL(VMOVLPS_MEMq_XMMq) = COPY_QWORD<M64W, V128>;
DEF_ISEL(VMOVLPS_XMMdq_XMMdq_MEMq) = VMOVQ_XMM_XMM_SRC2<MV64>;
#endif  // HAS_FEATURE_AVX

DEF_ISEL(MOVSD_XMM_XMMdq_MEMsd) = COPY_QWORD<V128W, MV64>;
DEF_ISEL(MOVSD_XMM_XMMsd_XMMsd_0F10) = LEGACY_COPY_QWORD<V128W, V128>;
DEF_ISEL(MOVSD_XMM_MEMsd_XMMsd) = COPY_QWORD<M64W, V128>;
DEF_ISEL(MOVSD_XMM_XMMsd_XMMsd_0F11) = LEGACY_COPY_QWORD<V128W, V128>;
#if HAS_FEATURE_AVX
DEF_ISEL(VMOVSD_XMMdq_MEMq) = COPY_QWORD<VV128W, MV64>;
DEF_ISEL(VMOVSD_MEMq_XMMq) = COPY_QWORD<M64W, V128>;
DEF_ISEL(VMOVSD_XMMdq_XMMdq_XMMq_10) = VMOVQ_XMM_XMM_SRC2<V128>;
DEF_ISEL(VMOVSD_XMMdq_XMMdq_XMMq_11) = VMOVQ_XMM_XMM_SRC2<V128>;
#endif  // HAS_FEATURE_AVX


DEF_ISEL(MOVSS_XMMdq_MEMss) = COPY_DWORD<V128W, MV32>;
DEF_ISEL(MOVSS_XMMss_XMMss_0F10) = LEGACY_COPY_DWORD<V128W, V128>;
DEF_ISEL(MOVSS_MEMss_XMMss) = COPY_DWORD<M32W, V128>;
DEF_ISEL(MOVSS_XMMss_XMMss_0F11) = LEGACY_COPY_DWORD<V128W, V128>;
#if HAS_FEATURE_AVX
DEF_ISEL(VMOVSS_XMMdq_MEMd) = COPY_DWORD<VV128W, MV32>;
DEF_ISEL(VMOVSS_MEMd_XMMd) = COPY_DWORD<M32W, V128>;
DEF_ISEL(VMOVSS_XMMdq_XMMdq_XMMd_10) = VMOVD_XMM_XMM_SRC2<V128>;
DEF_ISEL(VMOVSS_XMMdq_XMMdq_XMMd_11) = VMOVD_XMM_XMM_SRC2<V128>;
#endif  // HAS_FEATURE_AVX
