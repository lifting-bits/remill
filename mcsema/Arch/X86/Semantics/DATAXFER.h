...
DEF_SEM(MOVQ, D1 dst, S1 src_vec) {
  W(dst) = R(src_vec).qwords[0];  // Zero-extends.
}

template <typename D1, typename S1>
DEF_SEM(MOVQQ, D1 dst, S1 src_vec) {
  W(DownCast<V64W>(dst)) = R(src_vec).qwords[0];  // No zero-extension.
}
...
//       `vec <- vec` so that the low qword of the source operand is extracted,
//       resulting in zero assignment to the high qword of the XMM reg.
DEF_ISEL(MOVD_XMMdq_MEMd) = MOV<V128W, M32>;  // Zero extends.
DEF_ISEL(MOVD_XMMdq_GPR32) = MOV<V128W, R32>;  // Zero extends.
...
DEF_ISEL(MOVSD_XMM_XMMdq_MEMsd) = MOVQ<V128W, MV64>; // Implicit zext.
DEF_ISEL(MOVSD_XMM_XMMsd_XMMsd_0F10) = MOVQQ<V128W, V128>;  // Legacy, no zext.
DEF_ISEL(MOVSD_XMM_MEMsd_XMMsd) = MOVQ<M64W, V128>;
DEF_ISEL(MOVSD_XMM_XMMsd_XMMsd_0F11) = MOVQQ<V128W, V128>;  // Legacy, no zext.
